...
- *Inputs* — the decisions and settings written into a model. Results live in CSV
  exports that reference a run by its `id`.
- *The driver's vocabulary* — named ranges and the step each is written at, which
...
knows what the values *mean* lives in the injected policy layer, never here.
...
| `model.file` | **yes** | string | Model filename, resolved against the repo root. |
| `model.version` | no | string | Semver. Declared compatibility *intent*. |
...
place: semver is what a human *declares* about compatibility, the hash is what is
*true* about identity.
...
- **Excluding `model` is the load-bearing choice.** The id names *the decisions*,
...
- Key order cannot change an id: `{a:1,b:2}` and `{b:2,a:1}` are the same run.
  Array order can and must, because in `steps` the order *is* the timeline.
...
`seed` and `strategy` are worth calling out: they *explain* a file, they do not
*reconstruct* it. Re-running a seed against a different model version produces
...
>
> The day a tool wants to *branch* on `origin`, it has stopped being provenance and
...
- **Whether the run is *legal*.** Whether a decision was affordable, whether a
...
format *is*; that one records what it will become.
